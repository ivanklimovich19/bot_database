...
A web-based information system for storage, processing, and analysis of taxonomic
and morphological (phenomic) data of plants.

Developed for the Laboratory of Flora and Plant Systematics.
...
Install PostgreSQL 15 or newer from the official website:
...
(families, genera, species, traits).
...
Load Testing
The benchmark.py script reproduces the performance results reported in the
...
WARNING: the --seed flag adds data to the database. Do not run it on a
production database — use a separate test database.
...
Click New app, select the repository, branch main, and file app.py.

In the Secrets section, add the following variable:
...
Data Availability
The source code, database schema, and load testing script are available in this
repository. The deployed instance of the application operates within the
institutional secure network. An anonymised data subset is available upon
reasonable request.
...
Klimov I. A. Hybrid EAV architecture for storing plant phenotypic data:
implementation on the Python stack and performance evaluation //
Programming. 2026. [in press].
...
|-------------|--------------|
| `[your email]` | Your actual e-mail address |
| `https://github.com/ivanklimovich19/bot_database.git` | Actual repository URL if different |

---

## Recommended additional files

For the README to be accurate, add these files to the repository root:
...
Standard MIT License text. Get it from https://opensource.org/licenses/MIT.
...
git add README.md .env.example LICENSE .gitignore
git commit -m "Add README, .env template, LICENSE and .gitignore"
git push
After pushing, the README will automatically render on the repository's main page.
